...
$s_p_id            = htmlspecialchars('s_p_id');
...
<html lang="en">
  ...
  <div class="container-fluid">
    ...
    <?php 
     if(isset($_GET['id'])){
        $id  = $_GET['id'];
    if($product_cetagori!='' && $s_p_quentity!='' && $s_p_date!=''){
       $sql_select = "SELECT * FROM store_product WHERE store_product_id=$id";
       $sql_Q = $connect->query($sql_select);
       
       $con_array  =  mysqli_fetch_assoc($sql_Q);

       $store_product_id = $con_array['store_product_id'];
       $store_product_name = $con_array['store_product_name'];
       $store_product_quantity = $con_array['store_product_quantity'];
       $store_product_entry_date = $con_array['store_product_entry_date'];
      }
    }
      ?>
           ...
           <div class="row">
               ...
               <?php 
                   ...
                   if(isset($_GET['s_p_quentity'])){
                     ...
                     $sql_u = "UPDATE store_product SET store_product_name = '$product_c' , store_product_quantity= '$s_P_quen',
                     store_product_entry_date='$s_pro_date' WHERE store_product_id='$s_p_i'";
                  ...
                  ?>
                   <form action="<?php $_SERVER['PHP_SELF']; ?>" method="GET" class="shadow-lg p-3" style="color: #fff; background-color: #273c75; border-radius: 5px;">
                     <div class="text-form mb-5" style="text-align: center; color: #fff;">
                       <h1>Edit store product document</h1> 
                     </div>
                     <div class="mb-3">
                     ...
                     <div class="mb-3">
                       ...
                       <input type="number" class="form-control" id="product_code" name="s_p_quentity" value="<?php echo $store_product_quantity; ?>">
                     </div>
                     <div class="mb-3">
                       <label for="date" class="form-label">Store product date</label>
                       <input type="date" class="form-control" id="date" name="s_p_date" value="<?php echo $store_product_entry_date; ?>">
                       <input type="text" name="s_p_id" hidden value="<?php echo $store_product_id ?>">
                     </div>
